    ...
    public String[] getTokens(){
        ...
        for(int i = 0; i<data.length; i++){
            ...
            if(contadorEspacios == 0){ //Si es 0 podemos saber que termino la funcion
                ...
                String exp5 = "^[a-zA-Z]+\\s*\\({1}(((\\s*[a-zA-Z]+\\s*,\\s*(?=[a-zA-Z\"]))|(\\s*\"{1}\\s*[a-zA-Z:\\s]+\\s*\"{1}\\s*,\\s*(?=[a-zA-Z\"])))+)?((\\s*[a-zA-Z]+\\s*)|(\\s*\"{1}\\s*[a-zA-Z:\\s]+\\s*\"{1}\\s*))\\){1}$";
                ...
                if(itsFunction){//Dentro de una funcion
                    ...
                    String exp5 = "^\\s{4}print\\s*\\({1}(((\\s*[a-zA-Z]+\\s*,\\s*(?=[a-zA-Z\"]))|(\\s*\"{1}\\s*[a-zA-Z\\s]+\\s*\"{1}\\s*,\\s*(?=[a-zA-Z\"])))+)?((\\s*[a-zA-Z]+\\s*)|(\\s*\"{1}\\s*[a-zA-Z\\s]+\\s*\"{1}\\s*))\\){1}$";//Evalua el print correctamente 
                    //Evalua un return 
                    ...
                    if(Pattern.compile(exp4).matcher(data[i]).matches()||Pattern.compile(exp4v2).matcher(data[i]).matches()){
                        itsFunction = false;
                    }
                    
                    if(Pattern.compile(exp6).matcher(data[i]).matches()||Pattern.compile(exp5).matcher(data[i]).matches() ||Pattern.compile(exp4v2).matcher(data[i]).matches() ||Pattern.compile(exp).matcher(data[i]).matches() || Pattern.compile(exp2).matcher(data[i]).matches() || Pattern.compile(exp3).matcher(data[i]).matches()){
