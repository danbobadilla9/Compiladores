    ...
    public String[] getTokens(){
        ...
        for(int i = 0; i<data.length; i++){
            ...
            if(contadorEspacios == 0){ //Si es 0 podemos saber que termino la funcion
                ...
                if(Pattern.compile("^def\\s+[a-zA-Z]+\\({1}(\\s*(?=[a-zA-Z\\)])(([a-zA-Z0-9]+(\\s*,(?=[a-zA-Z0-9]))?|(\\s*,\\s*(?=[a-zA-Z]))?)+|[a-zA-Z0-9]+)?)\\s*\\){1}:{1}$").matcher(data[i]).matches()){
                    scanFuncion(data[i]);
                    itsFunction = true;
                }else if(Pattern.compile(exp).matcher(data[i]).matches() || Pattern.compile(exp2).matcher(data[i]).matches()|| Pattern.compile(exp3).matcher(data[i]).matches()){ //Si no es función se agrega al codigo MAIN
                ...
                if(itsFunction){//Dentro de una funcion
                    ...
                    String exp = "^((\\s{4}print\\({1}(\\s*(?=[a-zA-Z])(([a-zA-Z0-9]+(\\s*,(?=[a-zA-Z0-9]))?|(\\s*,\\s*(?=[a-zA-Z]))?)+|[a-zA-Z0-9]+)?)\\s*\\){1})||(\\s{4}[a-zA-z]+\\s*={1}\\s*\\[{1}[a-zA-Z]*\\]{1})||(\\s{4,8}for\\s*(?=[a-z])[a-zA-Z]\\s*(?=[a-z])in\\s*(?=[a-z])range\\({1}\\s*(?=[a-z])?[0-9]\\s*(?=[a-z])?,\\s*(?=[a-z])[a-zA-Z]+\\s*(?=[a-z\\)])\\){1}:{1}))$";
                    //Valida casteo, inputs,operacion vector suma
                    String exp2 = "^((\\s{4,8}[a-zA-Z]+\\s*(?=[a-z=])=\\s*(?=[a-z])((int)|(input))\\s*(?=[a-z\\(])\\({1}\\s*(?=[a-zA-Z])input\\s*(?=[\\(])\\({1}\\s*(?=[a-z\"])\"{1}[a-zA-Z\\s:]+\"{1}\\s*(?=[\\)])\\){1}\\s*(?=[\\)])\\){1})||(\\s{4,8}[a-zA-Z]+.append\\({1}\\s*[a-zA-Z]+\\s*\\){1})||(\\s{4,8}return\\s*(?=[a-zA-Z])[a-zA-Z]+(?!\\S))|((\\s{4,8}[a-zA-Z]+\\s*(?==)=\\s*(?=[a-zA-Z])[a-zA-Z]+\\s*(?=[a-zA-Z\\+])\\+\\s*(?=[a-zA-Z\\+])[a-zA-Z]+\\[{1}[a-zA-Z]\\]{1}(?!\\S))))$";
                    ...
                    if(Pattern.compile(exp4).matcher(data[i]).matches()){
                        itsFunction = false;
                    }
                    
                    if(Pattern.compile(exp).matcher(data[i]).matches() || Pattern.compile(exp2).matcher(data[i]).matches() || Pattern.compile(exp3).matcher(data[i]).matches()){
